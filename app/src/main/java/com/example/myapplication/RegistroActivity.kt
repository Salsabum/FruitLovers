...
class RegistroActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnGuardar.setOnClickListener {
            if (chkTyc.isChecked) {
                ...
                Toast.makeText(this, "Tiene que aceptar los tÃ©rminos y condiciones", Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun registrarUsuarioSQLite() {
        ...
        if (tietCorreo.text.toString().trim().isEmpty()) {
            tilCorreo.error = "Ingrese correo electrÃ³nico"
            error = true
        } else {
            tilCorreo.error = ""
        }
        if (tietClave.text.toString().trim().isEmpty()) {
            tilClave.error = "Ingrese contraseÃ±a"
            error = true
        } else {
            tilClave.error = ""
        }
        ...
        CoroutineScope(Dispatchers.Main).launch {
            val resultado = withContext(Dispatchers.IO) {
                ...
                val valores = ContentValues().apply {
                    ...
                    put("sexo", sexo)
                    put("correo", tietCorreo.text.toString().trim() + "@gmail.com")
    ...
    private fun registrarUsuarioFirebase() {
        ...
        val correoCompleto = if (correo.contains("@")) correo else "$correo@gmail.com"
        ...
        if (correo.isEmpty()) {
            ...
            error = true
        } else if (!correoCompleto.endsWith("@gmail.com", ignoreCase = true)) {
            tilCorreo.error = "Use una cuenta @gmail.com"
            error = true
        } else {
            tilCorreo.error = null
        }
        if (clave.isEmpty()) {
            tilClave.error = "Ingrese contrasena"
            error = true
        } else if (clave.length < 6) {
            tilClave.error = "La contrasena debe tener al menos 6 caracteres"
            error = true
        } else {
            tilClave.error = null
        }
        ...
        auth.createUserWithEmailAndPassword(correoCompleto, clave)
                ...
                if (task.isSuccessful) {
                    ...
                    db.child("usuarios").child(uid).setValue(usuarioMap)
                        .addOnSuccessListener {
                            ...
                            entrarAlSistema(auth, correoCompleto, clave)
                        }
                        .addOnFailureListener {
                            Log.e("Registro", "Cuenta creada, pero fallo guardar perfil", it)
                            Toast.makeText(this, "Cuenta creada, pero fallo guardar perfil", Toast.LENGTH_LONG).show()
                            entrarAlSistema(auth, correoCompleto, clave)
                        }
                } else {
                    val mensaje = when (task.exception) {
                        is FirebaseAuthUserCollisionException -> "Ese correo ya esta registrado, intentando iniciar sesion..."
                        is FirebaseAuthWeakPasswordException -> "La contrasena es muy debil"
                        is FirebaseAuthInvalidCredentialsException -> "Correo invalido. Use formato de Gmail"
                        else -> "No se pudo registrar: ${task.exception?.message ?: "error desconocido"}"
                    }
                    Toast.makeText(this, mensaje, Toast.LENGTH_LONG).show()
                    Log.e("Registro", mensaje, task.exception)

                    if (task.exception is FirebaseAuthUserCollisionException) {
                        auth.signInWithEmailAndPassword(correoCompleto, clave)
                            .addOnSuccessListener {
                                val intent = Intent(this, InicioActivity::class.java)
                                intent.putExtra("mostrarFragmento", "productos")
                            ...
                            .addOnFailureListener {
                                Toast.makeText(
                                    this,
                                    "Ese correo ya existe y la contrasena no coincide. Inicia sesion desde Acceso.",
                                    Toast.LENGTH_LONG
                                ).show()
                            }
                    }
    ...
    private fun entrarAlSistema(auth: FirebaseAuth, correo: String, clave: String) {
        ...
        auth.signInWithEmailAndPassword(correo, clave)
            ...
            .addOnFailureListener {
                Toast.makeText(this, "Registro correcto, pero no se pudo iniciar sesion automatico", Toast.LENGTH_LONG).show()
            }
    }
}
